...
interface CharacterData {
  ...
  joints?: Partial<
    Record<
      "head_top" | "neck" | "shl" | "shr" | "hipl" | "hipr" | "footl" | "footr",
      { x: number; y: number }
    >
  > | null;
...
const getSkinPath = (skinId?: string) =>
  ISLAND_SKINS.find((s) => s.id === skinId)?.imagePath ??
  ISLAND_SKINS[0].imagePath;
...
export default function App() {
  ...
  const [evolvingCharacter, setEvolvingCharacter] =
    useState<CharacterData | null>(null);
  ...
  useEffect(() => {
    ...
    const loadIslands = async () => {
      try {
        ...
        setIslands(norm);
        setNextIslandId(
          norm.length > 0 ? Math.max(...norm.map((i) => i.id)) + 1 : 1,
        );
        if (norm.length === 0) {
    ...
    const tick = (now: number) => {
      ...
      setCharacters((prev) => {
        ...
        const next = prev.map((ch) => {
          ...
          const nx = Math.max(
            ROAM_MIN_X,
            Math.min(ROAM_MAX_X, current.x + (dx / dist) * step),
          );
          const ny = Math.max(
            ROAM_MIN_Y,
            Math.min(ROAM_MAX_Y, current.y + (dy / dist) * step),
          );
  ...
  const zoomBy = useCallback((delta: number) => {
    const next = Math.min(
      Math.max(zoomRef.current + delta, MIN_ZOOM),
      MAX_ZOOM,
    );
  ...
  const handleWheel = useCallback(
    (e: React.WheelEvent<HTMLDivElement>) => {
      ...
      const next = Math.min(
        Math.max(zoomRef.current - e.deltaY * ZOOM_SENSITIVITY, MIN_ZOOM),
        MAX_ZOOM,
      );
      const ratio = next / zoomRef.current;
  ...
  const handleCanvasPointerDown = useCallback(
    (e: React.PointerEvent<HTMLDivElement>) => {
      ...
      if (["draw", "upload", "rig"].includes(modalState) || selectedCharacter)
        return;
      const t = e.target as HTMLElement;
      if (t.closest("[data-no-pan]") || t.closest("button") || t.closest("a"))
        return;
  ...
  const getPackedPos = (index: number) => {
    ...
    return {
      x: 50 + (Math.cos(a) * r) / WORLD_SCALE,
      y: 50 + (Math.sin(a) * r) / WORLD_SCALE,
    };
  };
  ...
  const getCharacterPosition = (islandId: number) => {
    ...
    for (let a = 0; a < 240; a++) {
      ...
      if (!existing.some((p) => Math.hypot(p.x - c.x, p.y - c.y) < minDist))
        return c;
    }
    return { x: 50, y: 50 };
  };
  ...
  const handleAddCharacter = (
    ...
    if (evolvingCharacter) {
      ...
      setRigPreviewUrl(
        imageFile ? URL.createObjectURL(imageFile) : pendingDrawing,
      );
      setModalState("rig");
      return;
    }
  ...
  const handleRigConfirm = async (
    ...
    const imageFile =
      evolutionPayload?.imageFile ?? creationPayload?.imageFile ?? null;
    const name = evolutionPayload?.name ?? creationPayload?.name ?? "";
    const age = evolutionPayload?.age ?? creationPayload?.age ?? 1;
    const islandId =
      evolutionPayload?.islandId ?? creationPayload?.islandId ?? 1;
    const personality = evolutionPayload?.personalityDelta ??
    ...
    try {
      ...
      if (evolutionPayload) {
        setCharacters((prev) =>
          prev.map((c) =>
            c.id === normalizedCharacter.id ? normalizedCharacter : c,
          ),
        );
      } else {
  ...
  if (loading) {
    return (
      ...
      >
        <p className="text-lg animate-pulse" style={{ color: txtMuted }}>
          Loading islands…
        </p>
      </div>
    );
    ...
    <div
      ...
      style={{
        backgroundImage: darkMode
          ? "linear-gradient(to bottom, #2673a8 0%, #0d2a47 30%, #0d2a47 100%)"
          : "linear-gradient(to bottom, #e8f9ff 0%, #8dd6f5 25%, #8dd6f5 100%)",
      }}
    ...
    >
      ...
      <div
        style={{
          ...
          transition:
            isPanning || draggingIslandId ? "none" : "transform 0.08s ease-out",
        }}
            ...
            >
              <div
                style={{
                  ...
                  outline: armed
                    ? "3px dashed rgba(59,130,246,0.9)"
                    : undefined,
              ...
              >
                {characters
                  ...
                  .map((ch) => {
                    ...
                    const direction: 1 | -1 =
                      !target || target.x >= ch.position.x ? 1 : -1;
                    const phaseSeed = ch.id
                      .split("")
                      .reduce((acc, c) => acc + c.charCodeAt(0), 0);
                    const roamCycle =
                      Math.floor(Date.now() / 2600 + phaseSeed) % 2;
                    const autoMode: RigAnimMode = !moving
                      ? "idle"
                      : roamCycle === 0
                        ? "walk"
                        : "hop";
      ...
      {islands.length === 0 && (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-5 pointer-events-none text-center px-6">
          ...
          <div>
            <p className="text-lg font-semibold" style={{ color: txtMain }}>
              No islands yet
            </p>
      ...
      >
        ...
        {islands.length > 0 && (
          <CtrlBtn
            onClick={() => flyToIsland(islands[0])}
            title="Go to home island"
          >
                  ...
                  <p className="text-sm text-gray-400 mt-0.5">
                    ...
                    <span className="font-medium text-gray-600">
                      {pendingCharacter?.name ??
                        evolvingCharacter?.name ??
                        "your character"}
                    </span>
                  </p>
                </div>
                <button
                  onClick={handleRigBack}
        ...
        >
          <div
            ...
            style={{
              width: MW,
              height: MH,
              background: darkMode ? "#0d1f2f" : "#f0f9ff",
            }}
          >
            {islands.map((isl, idx) => {
            ...
            {characters.map((ch) => {
              ...
              const cx =
                worldPositions[idx].x + ((ch.position.x - 50) / 100) * isl.size;
              const cy =
                worldPositions[idx].y +
                ((ch.position.y - 50) / 100) * (isl.size * 0.4);
            ...
            <div
              ...
              style={{
                left: vpX,
                top: vpY,
                width: Math.max(12, vpW),
                height: Math.max(12, vpH),
          ...
          <p
            ...
            style={{ color: darkMode ? "#6a8fa8" : "#aaa" }}
          >
            Map · click to expand
          </p>
        </div>
          ...
          >
            ...
            >
              <p
                ...
                style={{ color: darkMode ? "#9bb8cc" : "#555" }}
              >
            ...
            <div
              ...
              onPointerMove={(e) => {
                ...
                setExpPanX(
                  dragRef.current.spx + (e.clientX - dragRef.current.sx),
                );
                setExpPanY(
                  dragRef.current.spy + (e.clientY - dragRef.current.sy),
                );
              }}
              onPointerUp={(e) => {
            ...
            >
              <div
                ...
                style={{ background: darkMode ? "#0d1f2f" : "#dff3fb" }}
              />
              {islands.map((isl, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    />
                    <p
                      className="text-center mt-1 text-xs font-medium truncate"
                      style={{
                        color: darkMode ? "#9bb8cc" : "#444",
                        width: dw,
                      }}
                    >
                      {isl.label}
